...
public class MissingActionsValidationRuleTest {

  @Test(expected = IncorrectTestDefinitionException.class)
  public void validateEmptyActions() {
    TestCase aTestCase = new TestCase();

    aTestCase.setTestType(TestType.DATAMATCH.name());
    aTestCase.setTestName("Testing Empty Actions List");

    List<Action> actions = new ArrayList<Action>();

    aTestCase.setActions(actions);
    MissingActionsValidationRule rule = new MissingActionsValidationRule();
    rule.validate(aTestCase);
  }

  @Test(expected = IncorrectTestDefinitionException.class)
  public void validateNoActions() {
    TestCase aTestCase = new TestCase();

    aTestCase.setTestType(TestType.DATAMATCH.name());
    aTestCase.setTestName("Testing Missing Actions");
  ...
  @Test
  public void validateActionsSucess() throws IncorrectTestDefinitionException {
    TestCase aTestCase = new TestCase();

    aTestCase.setTestType(TestType.DATAMATCH.name());
    aTestCase.setTestName("A sucessfull test case with valid actions");
